...
for anno in annos:
  ...
  while neg_num < 50:
    ...
    if Iou[0] < 0.3:
      # print "Iou[0]: %d" % (Iou[0])
      save_path = os.path.join(neg_img_dir, '%d.xml' % n_id)
      f2.write("negative/%s.xml 0\n" % n_id)
  ...
  for i in range(20):
    ...
    if Iou[0] >= 0.65:
      save_path = os.path.join(pos_img_dir, '%s.xml' % p_id)
      f1.write('/positive/%d.xml 1\n' % p_id)
      f4.write('/positive/%d.xml %.2f %.2f %.2f %.2f\n' % 
      ...
      save_path = os.path.join(part_img_dir, '%s.xml' % a_id)
      f3.write('/part/%d.xml %.2f %.2f %.2f %.2f\n' % 
