...
base_dir = config.pro_img_dir + '/%d_xywh' %out_size
...
for anno in annos:
  ...
  for i in range(20):
    ...
    offset_w = (nx2 - nx1)/ float(x2-x1)
    offset_h = (ny2 - ny1)/ float(y2-y1)
    ...
    if Iou[0] >= 0.65:
      ...
      f4.write('positive/%d.npy %.2f %.2f %.2f %.2f\n' % 
        (p_id, offset_x1, offset_y1, offset_w, offset_h))
      # fs = cv2.FileStorage(save_path, cv2.FileStorage_WRITE)
      # fs.write('depth', resized_depth_im)
      # fs.release()
      np.save(save_path, resized_depth_im)
      ...
      f3.write('part/%d.npy %.2f %.2f %.2f %.2f\n' % 
        (a_id, offset_x1, offset_y1, offset_w, offset_h))
      np.save(save_path, resized_depth_im)
      # fs = cv2.FileStorage(save_path, cv2.FileStorage_WRITE)
      # fs.write('depth', resized_depth_im)
      # fs.release()
