...
def IoU(rect_1, rect_2):
    ...
    union = (x12 - x11) * (y12 - y11) + \
        (x22 - x21) * (y22 - y21) - intersection
    if union == 0:
        return 0
    else:
...
def NMS(rectangles, threshold, type):
    ...
    while cur_rect < number_of_rects:
        ...
        while rects_to_compare > 0:
            score = 0
            if type == 'iou':
                score = IoU(result_rectangles[cur_rect],
                            result_rectangles[cur_rect_to_compare])
            else:
                score = IoM(result_rectangles[cur_rect],
                            result_rectangles[cur_rect_to_compare])
            if score >= threshold:
...
def detect_face_12net(cls_prob, roi, out_side, scale, width, height, threshold):
    ...
    for (y, x), prob in np.ndenumerate(cls_prob):
        if(prob >= threshold):
            ...
            rect = []
            x1 = int(round(max(0, original_x1 + original_w * roi[0][y][x])))
            y1 = int(round(max(0, original_y1 + original_h * roi[1][y][x])))
            x2 = int(
                round(min(width, original_x2 + original_w * roi[2][y][x])))
            y2 = int(
                round(min(height, original_y2 + original_h * roi[3][y][x])))
            if x2 > x1 and y2 > y1:
...
def filter_face_24net(cls_prob, roi, rectangles, width, height, threshold):
    ...
    for i in range(rect_num):
        if cls_prob[i][1] > threshold:
            ...
            x2 = int(
                round(min(width, rectangles[i][2] + original_w * roi[i][2])))
            y2 = int(
                round(min(height, rectangles[i][3] + original_h * roi[i][3])))
            if x2 > x1 and y2 > y1:
...
def filter_face_18net(cls_prob, roi, rectangles, width, height, threshold):
    boundingBox = []
    rect_num = len(rectangles)
    for i in range(rect_num):
        if cls_prob[i][1] > threshold:
...
def filter_face_48net(cls_prob, roi, rectangles, width, height, threshold):
    ...
    for i in range(rect_num):
        if cls_prob[i][1] > threshold:
            ...
            boundingBox.append(rect)
    rectangles = NMS(boundingBox, 0.7, 'iou')
    rect = []
    ...
    for rectangle in rectangles:
        ...
        score = rectangle[4]
        rect_ = np.round([x1, y1, x2, y2]).astype(int)
...
def calculateScales(img):
    ...
    scales = []
    factor = 0.71
    while face_size <= max_face_size:
        current_scale = 24.0 / face_size
    ...
    # pr_scale = 1.0
    # scales = []
    # # scales.append(pr_scale)
    # factor = 0.7937
    # factor_count = 0
    # minl = min(h, w)
    # while minl >= min_face_size:
    #     # print minl
